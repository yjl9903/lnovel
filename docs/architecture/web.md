...
2. `/health`、`/bili`、`/bili/*`，把原始 Request 交给 `serverApp.fetch`，保留 API 状态码、响应体、缓存头及错误。
3. 生产静态资源，根目录由启动器设置为 `dist/client` 的绝对路径。
4. TanStack Start 页面与内部请求。

`src/server/api.server.ts` 创建一个进程内 API 容器，供网关和 SSR 共用。开发环境也调用此容器，不需要另外启动 HTTP API 服务。未知页面返回真实 404 和首页入口，不执行首页排行榜 loader。

`vite.config.ts` 将 `@lnovel/server` 保持为 SSR 外部依赖，并禁止客户端导入它或 `node:` 模块。浏览器产物不应包含数据库、抓取引擎或凭据配置。

## SSR、Query 与 Store

- `src/lib/top.ts` 定义共享 Query：key 为 `['top-weekvisit']`，`staleTime` 为 60 秒，接口为 `/bili/top/weekvisit`。
...
sitemap 只列出正式首页，不包含 API、RSS、外站地址或未经验证的更新时间。robots 允许首页和静态资源，声明禁止抓取业务接口、RSS、`/health` 和 Start 默认内部前缀 `/_serverFn`，同时放行封面所需的 `/bili/files/` 与 `/bili/img3/`。修改内部请求前缀时同步更新规则。
