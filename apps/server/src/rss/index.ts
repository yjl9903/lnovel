...
const globResultValidator = z.record(z.string(), z.function().output(z.promise(z.any())));
...
async function validateRssOptions(rssOptions: RSSOptions) {
  const parsedResult = await rssOptionsValidator.safeParseAsync(rssOptions, { error: errorMap });
  ...
  return Promise.all(
    Object.entries(items).map(async ([filePath, getInfo]) => {
      ...
      const parsedResult = rssSchema
        ...
        .safeParse({ ...frontmatter, link: url }, { error: errorMap });
